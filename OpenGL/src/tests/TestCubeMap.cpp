...
namespace test
{
		...
		: camera(&m_Proj, &m_View, 1280.0f, 768.0f)
	{
		...
		m_ModelShader = std::make_unique<Shader>("res/shaders/Cube.shader");

		m_SkyboxTexture = std::make_unique<CubeMap>("res/textures/skybox");

		loadModel = new obj::Model("res/obj/teapot/teapot.obj");
	...
	void TestCubeMap::OnRender()
	{
		...
		{
			/*m_ModelShader->Bind();
			m_Model = glm::translate(glm::mat4(1.0f), objPosition);
			loadModel->Draw(*m_ModelShader);
			...
			m_ModelShader->SetUniformVec3f("u_CameraPosition", camera.GetCameraPosition());*/
		}
	...
	void TestCubeMap::OnImGuiRender()
	{
		...
		ImGui::SliderFloat3("Object", &objPosition.x, -50.0f, 50.0f);
